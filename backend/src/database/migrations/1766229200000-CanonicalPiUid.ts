...
export class CanonicalPiUid1766229200000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      'UPDATE "users" SET "piWalletAddress" = NULL WHERE "piWalletAddress" = "piUid"',
    );
    await queryRunner.query(
      'CREATE UNIQUE INDEX IF NOT EXISTS "IDX_users_piUid_unique" ON "users" ("piUid") WHERE "piUid" IS NOT NULL',
    );
    await queryRunner.query(
      'CREATE INDEX IF NOT EXISTS "IDX_users_piUid_lookup" ON "users" ("piUid") WHERE "piUid" IS NOT NULL',
    );
  }
